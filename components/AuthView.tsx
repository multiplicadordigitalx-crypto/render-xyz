      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-black overflow-hidden flex-col justify-between p-16">
        ...
        <div className="relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <h2 className="text-6xl font-black text-white uppercase tracking-tighter leading-[0.9] mb-12">
              Arquitetura de <br /> <span className="text-neutral-500">Próximo Nível</span>
            </h2>
          </motion.div>
          <div className="space-y-8">
            {[
              ...
              >
                <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center shrink-0 border border-white/10 group-hover:bg-white transition-all">
        ...
        <div className="relative z-10 flex items-center justify-between border-t border-white/10 pt-8">
          ...
          <div className="text-right">
            ...
            <p className="text-neutral-500 text-[8px] font-bold uppercase tracking-[0.1em]">Processados com Sucesso</p>
          </div>
        </div>
      </div>

      {/* RIGHT SIDE: AUTH FORM */}
      <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 relative overflow-y-auto h-screen bg-neutral-100">
        {/* Animated Background */}
        <div className="absolute inset-0 z-0 overflow-hidden bg-neutral-100"><Boxes /></div>


        <button
          onClick={() => pendingGoogleUser ? setPendingGoogleUser(null) : onBack()}
          className="absolute top-6 left-6 flex items-center text-neutral-500 hover:text-black text-[9px] font-black uppercase tracking-[0.3em] transition-all z-20"
        >
              ...
              {!pendingGoogleUser ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    {(mode === 'login' || mode === 'register') && (
                      <>
                        <button
                          ...
                          className="w-full py-3.5 bg-white border border-neutral-200 rounded-[15px] font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-gray-50 transition-all flex items-center justify-center space-x-3 group relative overflow-hidden"
                        >
                        ...
                        <div className="relative flex items-center justify-center py-1">
                          ...
                          <span className="relative bg-white px-4 text-[9px] font-black text-neutral-400 uppercase tracking-widest">OU</span>
                        </div>
                      </>
                    )}
                    {mode === 'register' && (
                      <div className="relative group">
                        <User className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 group-focus-within:text-black transition-colors" />
                        <input type="text" required placeholder="NOME COMPLETO" className="w-full pl-14 pr-6 py-3.5 bg-neutral-100 border border-transparent rounded-2xl text-[10px] font-bold focus:bg-white focus:border-neutral-300 focus:outline-none transition-all placeholder:text-neutral-400" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                      </div>
                    )}
                    {pendingPlanName && mode === 'register' && (
                      <div className="mb-4 p-3 bg-black/5 rounded-xl flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[9px] font-bold uppercase text-neutral-500 tracking-wider">Ativando Plano</p>
                          <p className="text-xs font-black uppercase tracking-widest">{pendingPlanName}</p>
                        </div>
                      </div>
                    )}
                    <div className="relative group">
                      <Mail className={`absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${lockedEmail ? 'text-black' : 'text-neutral-400 group-focus-within:text-black'}`} />
                      <input
                        type="email"
                        required
                        placeholder="E-MAIL"
                        ...
                        className={`w-full pl-14 pr-6 py-3.5 border rounded-2xl text-[10px] font-bold outline-none transition-all placeholder:text-neutral-400 ${lockedEmail
                          ? 'bg-black/5 border-black/10 text-black cursor-not-allowed opacity-70'
                          : 'bg-neutral-100 border-transparent focus:bg-white focus:border-neutral-300'
                          }`}
                    ...
                    {mode === 'register' && (
                      <div className="relative group">
                        <ShieldCheck className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 group-focus-within:text-black transition-colors" />
                        <input type="text" required placeholder="CPF" className="w-full pl-14 pr-6 py-3.5 bg-neutral-100 border border-transparent rounded-2xl text-[10px] font-bold focus:bg-white focus:border-neutral-300 focus:outline-none transition-all placeholder:text-neutral-400" value={formData.cpf} onChange={handleCpfChange} />
                      </div>
                    )}
                    {mode !== 'forgot-password' && (
                      <div className="relative group">
                        <Lock className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 group-focus-within:text-black transition-colors" />
                        <input type="password" required placeholder="SENHA DE ACESSO" className="w-full pl-14 pr-6 py-3.5 bg-neutral-100 border border-transparent rounded-2xl text-[10px] font-bold focus:bg-white focus:border-neutral-300 focus:outline-none transition-all placeholder:text-neutral-400" value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} />
                      </div>
                    )}
                    {mode === 'register' && (
                      <div className="space-y-4">
                        <div className="relative group">
                          <Lock className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 group-focus-within:text-black transition-colors" />
                          <input type="password" required placeholder="CONFIRMAR SENHA" className="w-full pl-14 pr-6 py-3.5 bg-neutral-100 border border-transparent rounded-2xl text-[10px] font-bold focus:bg-white focus:border-neutral-300 focus:outline-none transition-all placeholder:text-neutral-400" value={formData.confirmPassword} onChange={e => setFormData({ ...formData, confirmPassword: e.target.value })} />
                        </div>
                        <div className="flex items-start space-x-3 p-2 bg-neutral-100/50 rounded-xl hover:bg-neutral-100 transition-colors cursor-pointer" onClick={() => setFormData({ ...formData, terms: !formData.terms })}>
                          ...
                          <label className="text-[8px] font-bold text-neutral-500 uppercase tracking-widest leading-relaxed cursor-pointer">Eu li e concordo com os <span className="text-black underline cursor-pointer hover:text-amber-600 transition-colors" onClick={(e) => { e.preventDefault(); setShowTerms(true); }}>Termos de Uso</span>.</label>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="mt-6 pt-4 border-t border-neutral-200 text-center">
                    {mode === 'login' ? (
                      <div className="space-y-3">
                        <button onClick={() => setMode('register')} className="text-[10px] font-black text-neutral-500 uppercase tracking-widest hover:text-black transition-all">Novo aqui? <span className="underline">Criar conta grátis</span></button>
                        <br />
                        <button onClick={() => setMode('forgot-password')} className="text-[9px] font-black text-neutral-400 uppercase tracking-widest hover:text-black transition-all">Esqueci minha senha</button>
                      </div>
                    ) : (
                      <button onClick={() => setMode('login')} className="text-[10px] font-black text-neutral-500 uppercase tracking-widest hover:text-black transition-all">Já tem conta? <span className="underline">Entrar agora</span></button>
                    )}
                  </div>
                </motion.div>
              ) : (
                <motion.div key="google-cpf" variants={variants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
                  <div className="text-center mb-6">
                    ...
                    <p className="text-neutral-500 text-[9px] font-black uppercase tracking-widest">Confirme seu CPF para continuar.</p>
                  </div>
                  ...
                  <form onSubmit={handleGoogleCompletion} className="space-y-4">
                    <div className="relative group">
                      <ShieldCheck className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 group-focus-within:text-black transition-colors" />
                      <input type="text" required placeholder="SEU CPF" className="w-full pl-14 pr-6 py-3.5 bg-neutral-100 border border-transparent rounded-2xl text-[10px] font-bold focus:bg-white focus:border-neutral-300 focus:outline-none transition-all placeholder:text-neutral-400" value={cpfForGoogle} onChange={e => handleCpfChange(e, true)} />
                    </div>
                    <button type="submit" disabled={loading} className="w-full py-4 bg-black text-white rounded-[15px] font-black text-[11px] uppercase tracking-[0.3em] shadow-xl hover:bg-zinc-800 transition-all flex items-center justify-center space-x-3 disabled:bg-zinc-300 disabled:cursor-not-allowed group">
